...
public class MachineLearningExperiments {
	...
	public static void main(String[] args) {
		...
		Mat trainingDataMat = new Mat(4, 2, CV_32FC1);
		trainingDataMat.put(new Mat(trainingData).reshape(0, 4));

		Mat labelsMat = new Mat(4, 2, CV_32FC1);
		labelsMat.put(new Mat(labels).reshape(0, 4));

		Mat labelsMat2 = new Mat(4, 1, CV_32SC1);
		labelsMat2.put(new Mat(labels2).reshape(0, 4));

		Mat labelsMat3 = new Mat(4, 1, CV_32FC1);
		labelsMat3.put(new Mat(labels3).reshape(4, 1));

		Mat sampleMat = new Mat(2, 2, CV_32FC1);
		sampleMat.put(new Mat(test).reshape(0, 2));
...
*/
	...
	/*public static Mat MyAnn(Mat trainingData, Mat labels, Mat testData) {
		...
		TrainData td = TrainData.create(trainingData, ROW_SAMPLE, labels);
		Mat layerSizes = new Mat(1, 4, CV_32FC1);
		...
		boolean success = ann.train(td.getSamples(), ROW_SAMPLE, td.getResponses());
	...
	public static Mat MySvmsgd(Mat trainingData, Mat labels, Mat testData) {
		...
		for (int i = 0; i < responseMat.arrayHeight(); i++) {
			if (responseMat.col(0).row(i).cols() == 0)
				System.out.println("Boy\n");
/*
			if (responseMat.get(i, 0)[0] == 1)
				System.out.println("Girl\n");
